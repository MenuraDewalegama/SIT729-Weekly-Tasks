...
const fire_warning_levels = ["NO RATING", "MODERATE", "HIGH", "EXTREME", "CATASTROPHIC"];

const client = net.createConnection(port, host, () => {
    console.log("Connected");
    setInterval(() => {
        const random_area = areas[Math.floor(Math.random() * areas.length)];
        const random_fire_warning_levels = fire_warning_levels[Math.floor(Math.random() * fire_warning_levels.length)];
        client.write(`fire, ${random_fire_warning_levels}, ${random_area}`);
